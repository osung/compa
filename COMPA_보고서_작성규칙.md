...
- **러닝 헤더/푸터 (현재 수요기술명 표시)**: 상단 좌측에 `▍` 강조바 + **현재 수요기술명**(그 페이지가 속한 수요), 우측 로고, 하단 얇은 괘선(좌측 끝 틸 강조 틱). 하단 가운데 `— PAGE —`. 구역1(표지·목차)은 헤더/푸터 없음.
  ...
  - **PDF(reportlab)**: `BaseDocTemplate.afterFlowable` 에서 수요 H2 flowable(`_demand=(번호,수요기술명)`)을 만나면 현재 수요를 기록, `onPage` 헤더에서 `수요 N` 라운드 배지(틸)+수요기술명(네이비)로 그림. 폭 초과 시 말줄임(…). 상세 페이지에서 현재 수요기술명이 항상 최상단에 노출됨.
...
- **헤더↔본문 여백(여유)**: 헤더가 본문에 너무 붙지 않도록 **상단 여백을 키움**. docx `top_margin=1.02in`(header_distance 0.35in), PDF `TM=28mm`이되 헤더 괘선은 페이지 위쪽 `HDR_Y=PAGE_H−15mm`에 그려 본문 프레임과 ~13mm 간격 확보. 4열 정보표로 확보한 세로 여유로 1페이지 수렴 유지.
