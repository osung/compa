...
- **러닝 헤더/푸터**: 상단 좌측 러닝헤더(`COMPA 기술수요–공공 R&D 매칭 보고서`, 하단 얇은 괘선), 하단 가운데 `— PAGE —`(네이비 굵게). 구역1(표지·목차)은 헤더/푸터 없음.
...
- 세리프 본문이 산세리프보다 높이가 커, 과제 상세 조판값을 **본문 8.6pt · 줄간격 1.1 · 섹션간격 2pt · 여백 상하 0.7in/좌우 0.9in** 로 재설정(최장 근거 ≈2,300자까지 1페이지 수렴).
